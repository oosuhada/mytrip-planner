# Kyoto · Osaka 2026 — Budget Trip Master Plan

Updated: 2026-09-10  
Trip: 2026-09-13 → 2026-09-17  
Travelers: Oosu + Domenic  
Canonical status: **this file is the human-readable source of truth for the travel plan.** Confirmed flight/hotel booking data in the production SQLite remains authoritative when there is any conflict.

---

## 0. Trip principles

- **Budget trip. No taxis.** Use train / subway / bus / walking only.
- Normal walking target: **7,000–10,000 steps/day**. Do not turn a food/rest trip into 15k–20k-step checklist tourism.
- Wake around 07:00, convenience-store breakfast, leave the hotel around 08:00.
- Plan as if it will rain. If rain becomes heavy, **delete one outdoor sight instead of adding effort**.
- Food, rest, and a few core sights outrank the number of attractions visited.
- Sushi at least once every day. Normal sushi budget: **about ¥2,000–¥4,000/person or less**.
- Dotonbori is a repeatable food/snack/shopping base. Visiting by day and again at night is intentional.
- Arashiyama is not a must and is excluded from the core itinerary.
- AMANEK was intentionally selected after checking tattoo-related guest reports. **Do not search for an external tattoo sauna or replacement hotel.**

### Food restrictions

- **Domenic:** sushi/fish is fine; avoid uni, shellfish/crustaceans and other non-fish seafood; avoid offal.
- **Oosu:** avoid spicy food and spicy sauces/toppings.
- At conveyor sushi, default to tuna, salmon, white fish and other clearly identifiable fish. Inspect gunkan/toppings before ordering.

---

# 1. Confirmed bookings

## Flights

### Sunday, September 13

- T’way Air — **ICN → KIX**
- 08:00 → 10:05

### Thursday, September 17

- T’way Air — **KIX → ICN**
- 11:35 → 13:30

## Hotels

### Kyoto — HOTEL AMANEK Kyoto Kawaramachi Gojo

- Stay: **9/13 → 9/15**
- Check-in: from 15:00
- Check-out: 9/15 11:00
- Address: **616 Azuchicho, Shimogyo Ward, Kyoto**

### Osaka — Nipponbashi Crystal Hotel

- Stay: **9/15 → 9/17**
- Address: **4-8-13 Nipponbashi, Naniwa Ward, Osaka**

---

# 2. Day-by-day itinerary

## 9/13 Sun — KIX → Kyoto / easy arrival day

**Theme:** flight + hotel + first sushi + covered arcade + bath. Do not add serious sightseeing.  
**Walking target:** about **6k–8k steps**.

| Time | Plan | Transport / notes |
|---|---|---|
| 05:45–05:52 | **Home → Hongik Univ. AREX** | Walk from 10 Wausan-ro 38-gil (Donggyo-dong 188-6); about 7 min normally. This deliberately targets the 05:57 train, so rain/luggage leaves little platform buffer |
| 05:57–06:52 | **AREX · Hongik Univ. → ICN T1** | All-stop train. User-selected departure; online check-in is assumed complete |
| 06:52–07:35 | **ICN T1 security / departure immigration / gate** | No currency exchange or airline-counter errand planned. Go directly to security; the trade-off is a tighter airport buffer |
| 08:00–10:05 | T’way ICN → KIX | Confirmed booking |
| 10:05–10:55 | **KIX T1 immigration / arrival** | Visit Japan Web QR ready; allow 30–60 min variation. Collect checked baggage here only if applicable |
| 10:55–11:10 | **KIX T1 → Kansai-airport Station** | Walk via the Terminal 1 2F / AEROPLAZA direct connection |
| ~11:15–12:30 | **HARUKA KIX → Kyoto Station** | About 75 min. 11:15 is a target; take the next train if immigration runs long. Official one-way adult baseline ¥2,200 |
| 12:35–13:00 | **Kyoto Station → Kawaramachi Gojo** | Default: Kyoto City Bus **4 / 5 / 80 / 205**, whichever is convenient; hotel official access says about 10 min onboard. Allow extra wait/traffic time. Standard flat-zone bus fare ¥230 |
| 13:00–13:03 | **Kawaramachi Gojo → AMANEK** | Hotel official access says about 1 min walk |
| 13:05–13:25 | AMANEK bag drop / ask about early check-in | Confirmed check-in remains at 15:00 |
| 15:00 | Check-in / short rest | Confirmed hotel |
| 16:30–17:30 | **Kura Sushi Kyoto Teramachi** | **RESERVE NOW**; official web/app |
| 17:30–19:30 | Teramachi / Shinkyogoku / Nishiki area | Slow covered-arcade walk; no Kiyomizu/Fushimi today |
| 20:30–21:30 | **AMANEK public bath** | Planned evening bath slot |
| after 21:30 | Convenience-store drink/snack / sleep | Recovery first |

### Heavy-rain version

- Keep HARUKA and subway transfer.
- Go straight to the hotel rather than walking around Kyoto Station.
- Keep Kura, then stay almost entirely inside Teramachi/Shinkyogoku covered arcades.
- Bath remains unchanged.

---

## 9/14 Mon — Kyoto core sightseeing day

**Theme:** Kiyomizu → Higashiyama/Gion → sushi → rest → bath → ramen.  
**Walking target:** about **8k–10k steps**; this is intentionally the highest-walking day.

| Time | Plan | Transport / notes |
|---|---|---|
| 07:00 | Wake | Convenience-store breakfast |
| 08:00–08:30 | Hotel → Kiyomizu area | Kyoto City Bus toward Gojo-zaka / Kiyomizu-michi + short uphill walk |
| 08:30–10:00 | **Kiyomizu-dera** | Core Kyoto sight; ticket on site |
| 10:00–11:10 | Sannenzaka → Ninenzaka → Yasaka Pagoda area → Gion | Mostly downhill; do not chase every photo spot |
| 11:15–12:00 | **Sushiro Kyoto Gion** | **RESERVE NOW**; official app/LINE |
| 12:00–14:30 | Gion / Kawaramachi | Weather-dependent relaxed block |
| 15:00–16:20 | Hotel rest | Required recovery block |
| 16:30–17:30 | **AMANEK public bath** | Planned second bath slot |
| 18:15–19:15 | **Ramen YUCHO** | **WALK-IN ONLY**; basic ginger-shoyu; avoid spicy options |
| after 19:15 | Dessert / convenience-store drink / rest | No extra temple/night route |

### Heavy-rain version

- Kiyomizu remains the core Kyoto sight, but shorten the temple and steep outdoor lanes.
- Skip most Sannenzaka/Ninenzaka lingering and move toward Gion/Sushiro earlier.
- Replace long Gion walking with department store + Teramachi/Shinkyogoku covered arcades.
- Keep the 15:00 rest and 16:30 bath.

---

## 9/15 Tue — conditional Fushimi Inari → Osaka → Dotonbori

**Theme:** one optional outdoor Kyoto block, then move city and use Dotonbori as the main Osaka base.  
**Walking target:** about **8k–10k steps**.

| Time | Plan | Transport / notes |
|---|---|---|
| 07:00 | Wake / breakfast | Check rain first |
| 08:30–09:40 | **Fushimi Inari — lower shrine only, CONDITIONAL** | Keihan Kiyomizu-Gojo ↔ Fushimi-Inari; lower shrine + beginning of Senbon Torii only; **no mountain climb** |
| ~10:15 | Return hotel | Pack / rest |
| 11:00 | AMANEK checkout | Confirmed booking |
| 09:40–10:15 | **Fushimi-Inari → AMANEK** | Keihan back to Kiyomizu-Gojo, then walk to hotel. Delete with Fushimi in heavy rain |
| 11:05–11:15 | **AMANEK → Kiyomizu-Gojo Station** | Walk with luggage after checkout |
| 11:20–12:10 | **Keihan Kiyomizu-Gojo → Kitahama** | Ordinary/express rail; transfer if required by train type; no Shinkansen |
| 12:10–12:25 | **Osaka Metro Kitahama → Ebisucho** | Sakaisuji Line |
| 12:25–12:35 | **Ebisucho → Nipponbashi Crystal Hotel** | Short walk with luggage |
| 12:35–13:00 | Nipponbashi Crystal Hotel | Leave bags / check room availability |
| 14:00–14:45 | **Genrokuzushi Dotonbori** | **WALK-IN ONLY**; first Osaka sushi |
| 14:45–15:50 | Dotonbori / Namba in daylight | Food/shopping base, not a one-time photo stop |
| 16:00–16:30 | **Rikuro’s Namba Main Store** | **WALK-IN ONLY**; cafe L.O. 16:30, otherwise takeout |
| ~16:30–18:00 | Hotel check-in / rest | Keep energy for evening |
| 18:30–19:45 | **CHIBO Dotonbori** | **RESERVE NOW**; official site → EBIca; share okonomiyaki + yakisoba |
| 20:00–21:30 | Dotonbori at night | Glico / river area / snacks / shopping |

### Heavy-rain version

- **Delete Fushimi Inari entirely.** Slow morning, checkout at 11:00.
- Keep the rail transfer to Osaka.
- In Osaka, shorten riverside walking and use Namba/Dotonbori/Shinsaibashi covered shopping areas.
- Keep Genroku, Rikuro and CHIBO because the day’s value is food + Dotonbori rather than outdoor sightseeing.

---

## 9/16 Wed — Osaka Castle + Shinsaibashi + food day

**Theme:** one major sight, then covered shopping and planned meals.  
**Walking target:** about **8k–10k steps**.

| Time | Plan | Transport / notes |
|---|---|---|
| 07:00 | Wake / convenience-store breakfast | — |
| 08:00–08:50 | Nipponbashi → Osaka Castle area | Osaka Metro; one transfer is acceptable |
| 09:00–10:45 | **Osaka Castle** | Buy admission on the day; museum interior is useful in rain |
| 11:30–12:30 | **Yamamoto no Hamburg Shinsaibashi** | **WALK-IN ONLY**; arrive before lunch queue grows |
| 13:00–14:30 | Shinsaibashi covered arcade | Slowly head south; shop/rest rather than detour |
| 15:00–15:45 | **Daiki Suisan Kaitenzushi Dotonbori** | **WALK-IN ONLY**; off-peak; keep spend within sushi budget |
| 16:00–18:00 | Hotel rest | Do not add another attraction |
| 19:00–20:00 | **Osaka Ohsho Nipponbashi** | **WALK-IN ONLY**; gyoza + fried rice/non-spicy noodles |
| 20:00–21:00 | Optional final Dotonbori night | Delete if wet/tired |

### Heavy-rain version

- Keep Osaka Castle but reduce the outdoor park loop; make the museum the anchor.
- Shinsaibashi covered arcade becomes the main walking route.
- Delete the optional final Dotonbori loop first.

---

## 9/17 Thu — Osaka → KIX / final sushi / home

**Theme:** airport buffer first.  
**Walking target:** about **3k–5k steps**.

| Time | Plan | Transport / notes |
|---|---|---|
| 06:30–06:45 | Wake | Final packing |
| 07:05–07:15 | **Nipponbashi Crystal Hotel → Ebisucho** | Walk with luggage |
| 07:15–07:25 | **Osaka Metro Ebisucho → Tengachaya** | Sakaisuji Line |
| 07:30–08:15 | **Nankai Airport Express Tengachaya → KIX** | Rapi:t only if disruption/timing risk makes it necessary |
| 08:15–08:25 | **Kansai-airport Station → KIX T1** | Direct indoor connection |
| 08:25–08:40 | KIX T1 departure floor / boarding-pass check | Assumes online check-in is already complete and there is no counter errand |
| 09:00–09:30 | **Osaka Tenma Sushi Nishiya** | **WALK-IN ONLY**, KIX T1 2F **before security** |
| 09:30–10:40 | Security / immigration / gate | Do not linger after sushi |
| 11:35–13:30 | T’way KIX → ICN | Confirmed booking |
| 13:30–14:15 | ICN T1 immigration / arrival | Collect checked baggage only if applicable |
| 14:15–15:05 | **AREX ICN T1 → Hongik Univ.** | All-stop train, about 50 min |
| 15:05–15:15 | **Hongik Univ. → home** | Walk about 7–10 min |

---

# 3. Restaurant master list

Official store/booking systems take precedence over aggregators when they conflict. Re-check temporary closure notices on 9/12.

| Restaurant | Plan | Current hours | Typical budget | Reservation | Channel |
|---|---|---|---|---|---|
| **Kura Sushi Kyoto Teramachi** | 9/13 16:30 | 11:00–23:00 | ~¥1,000–¥3,000/person | **RESERVE NOW** | Official web seat reservation / Kura official app |
| **Sushiro Kyoto Gion** | 9/14 11:15 | Weekday 11:00–23:00; weekends/holidays 10:30–23:00; L.O. 30m before | ~¥1,000–¥3,000/person | **RESERVE NOW** | Sushiro official app / official LINE reception/reservation |
| **Ramen YUCHO** | 9/14 18:15 | **11:00–22:00** | ~¥1,000–¥2,000/person | **WALK-IN ONLY** | Walk-in; no confirmed official online reservation inventory |
| **Genrokuzushi Dotonbori** | 9/15 14:00 | 11:00–22:30 | ~¥1,000–¥3,000/person | **WALK-IN ONLY** | Walk-in; reservations listed unavailable |
| **Rikuro’s Namba Main Store** | 9/15 16:00 | 1F 09:00–20:00; cafe 11:00–17:30, L.O. 16:30 | ~¥1,000/person dessert | **WALK-IN ONLY** | Walk-in / takeout; official site says no store/online/phone reservation service |
| **CHIBO Dotonbori** | 9/15 18:30 | 11:00–23:00; normal L.O. 22:00 | ~¥2,000–¥4,000/person | **RESERVE NOW** | Official CHIBO page → EBIca reservation |
| **Yamamoto no Hamburg Shinsaibashi** | 9/16 11:30 | 11:00–22:00; food L.O. 21:30 | ~¥2,000–¥3,000/person | **WALK-IN ONLY** | Walk-in; reservations unavailable |
| **Daiki Suisan Kaitenzushi Dotonbori** | 9/16 15:00 | 11:00–23:00 | ~¥2,000–¥4,000/person | **WALK-IN ONLY** | No seat reservations; at congestion some stores support app/LINE queue reception |
| **Osaka Ohsho Nipponbashi** | 9/16 19:00 | **Official: Sun–Thu 11:00–24:00 (L.O. 23:30); Fri/Sat 11:00–25:00 (L.O. 24:00); no regular holiday** | ~¥1,000–¥2,000/person | **WALK-IN ONLY** | Walk-in |
| **Gyoza no Ohsho Nihombashi-Denden-Town** | 9/16 dinner option | Mon–Sat 11:00–25:00 (L.O. 24:45); Sun/holidays 11:00–24:15 (L.O. 24:00) | ~¥1,000–¥2,000/person | **WALK-IN ONLY** | Walk-in; second gyoza option in the hotel/Nipponbashi area |
| **Osaka Tenma Sushi Nishiya — KIX** | 9/17 09:00 | 07:00–22:00 | ~¥1,000–¥3,000/person | **WALK-IN ONLY** | Walk-in; KIX T1 2F before security |

### Important corrections from older listings

- **Ramen YUCHO:** use the newer **11:00–22:00** public listing; do not plan around the older 21:30 closing time.
- **Osaka Ohsho Nipponbashi:** an older Tabelog listing says 11:00–22:00 / Tuesday closed. The **current official Osaka Ohsho store page** instead lists the later hours above and no regular holiday, so the official listing is canonical here.

### Reservation state at plan creation

- Restaurant planning is now **meal-slot based**, not a single permanently fixed restaurant per meal.
- Key lunch/dinner slots expose **2–3 candidates** in MyTrip; choosing one updates the linked schedule event.
- The reservation checklist only requires **BOOKED** for the restaurant currently selected in a meal slot when that restaurant is marked `RESERVE NOW`.
- Restaurant candidates are shared with **Candidates · Voting**, including vote count, Google Maps, menu/photo link and available official thumbnail.
- Current defaults remain Kura Kyoto Teramachi, Sushiro Kyoto Gion, Ramen YUCHO, Genrokuzushi, CHIBO, Yamamoto no Hamburg, Daiki Suisan, Osaka Ohsho and KIX Nishiya until changed in the site.

Additional alternatives currently linked in MyTrip include **Sushi no Musashi Sanjo Honten**, **Ramen Sen no Kaze Kyoto**, **Kura Sushi Dotonbori Global Flagship**, **Ajinoya Honten**, **Fukuyoshi Osaka Shinsaibashi** and **Gyoza no Ohsho Nihombashi-Denden-Town**. A candidate changing in the site is a user decision; the canonical route/time block remains unless the linked meal slot is changed.

### MyTrip decision-board behavior

- The site separates each date into real decision points instead of one undifferentiated candidate list: transport, meal, after-meal activity, recovery and after-dinner choices.
- On 9/13 this explicitly includes **KIX → Kyoto**, **Kyoto Station → AMANEK**, the first meal, the activity after that meal, the later meal/flex slot and the evening-after choice.
- Selecting an option does **not** delete the alternatives. The chosen option is emphasized and the others remain visible as **PLAN B** so they can be compared or re-selected later if weather, queues or fatigue change.
- The schedule itself shows the currently selected route; auxiliary cards that only belong to another route may be hidden there to keep the actual timeline coherent. Their full Plan B details remain in the decision board.
- The Day Plan has two explicit views on both desktop and mobile: **날짜별** renders one selected day at full width, while **전체보기** keeps all five wide Day cards in chronological order with horizontal scrolling. Before/after the trip 날짜별 opens on **Day 1**; during the trip it opens directly on the current Japan-date day.
- Schedule times are displayed and edited in **24-hour HH:mm** format.
- On mobile, the six-tab strip is intentionally removed. A compact **hamburger → left drawer** owns global navigation, while the Schedule screen uses a sticky **Day 1–5 date switcher** and renders only the selected day. This avoids horizontal day scrolling on a phone.
- The floating assistant opens as a **full-screen mobile sheet** (100dvh) so long itinerary-aware answers have enough reading space; desktop keeps the smaller floating panel.
- Itinerary cards always have a visual area. Restaurant/attraction/hotel/rail imagery uses official or official-source-page images where stable; otherwise the visual becomes an explicit Google Maps-linked route/place placeholder rather than an empty/broken image. Google Maps photos are not hotlinked without a configured Places API key.
- The application has two explicit experiences over the same source-of-truth data: **PLAN** for pre-trip research, comparisons, reservations, packing, voting and itinerary editing; **TRIP** for the Japan-local current day, immediate next actions, Maps, selected meals, Plan B changes and on-the-road AI help. Before the trip PLAN is the default; during 9/13–9/17 TRIP is the default, with manual switching always available.
- TRIP mode is split into separate field-use screens: **오늘**, **날씨 · 오늘의 코디**, **일본어 표현**, **전체 일정**, and **지도**. 오늘 only keeps NOW/NEXT, meals/reservations and Plan B; weather/outfit and language phrases are no longer mixed into that screen.
- **일본어 표현** is grouped into restaurant, transport, hotel and help phrases. Every phrase shows Japanese text, Korean phonetic reading and Korean meaning; tapping a phrase copies the Japanese text for showing to staff. **날씨 · 오늘의 코디** combines the current-day forecast, outfit guidance, carry-now items and the compact trip forecast.

### Expanded research pool — 2026-09-10

MyTrip now keeps a larger **research pool separate from the confirmed itinerary**. A researched candidate can be dated, voted and compared without silently becoming a scheduled stop. The candidate screen should be read as **date → Kyoto/Osaka → schedule order or theme**, while the canonical itinerary remains the route/time source of truth until Oosu explicitly selects/adds an option.

- **9/13 Kyoto arrival:** after the first sushi slot, optional 19:00 choices include **Katsukura Shijo Teramachi** (tonkatsu, 11:00–21:00), **MACCHA HOUSE Kyoto Kawaramachi** (matcha dessert, 11:00–20:30) and **Ramen Sen no Kaze**. Nearby non-food research includes **Shosei-en** and **Higashi Hongan-ji** between Kyoto Station/Gojo, plus **Pontocho** as a short evening walk. These are options, not obligations; arrival-day rest remains higher priority.
- **9/14 Kyoto:** keep Kiyomizu/Gion as the core. Add **Kennin-ji** as an easy Gion-adjacent temple, **Kyoto International Manga Museum** as a heavy-rain indoor substitute, and **Saryo Tsujiri Gion / MACCHA HOUSE** as dessert candidates rather than forcing one cafe into the schedule.
- **9/15 Osaka:** Dotonbori lunch/dinner now has broader alternates including **Mizuno** for okonomiyaki and **Dotonbori Imai / Tsurutontan Soemoncho** for an optional noodle stop. Nearby **Kuromon Market, Hozenji Yokocho and Namba Parks Garden** are short candidates around the hotel/Namba cluster.
- **9/16 Osaka:** preserve Osaka Castle + Shinsaibashi as the core. **Osaka Museum of Housing and Living** is a stronger indoor rain alternative; **Namba Yasaka, Kuromon, Hozenji and Namba Parks** remain short optional inserts rather than extra mandatory walking.

Research sources used for the expanded pool include official/current pages where available:

- Katsukura Shijo Teramachi: https://www.katsukura.jp/shops/sijoteramachi/
- MACCHA HOUSE Kyoto Kawaramachi: https://maccha-house.com/1174/en/
- Saryo Tsujiri Gion: https://www.giontsujiri.co.jp/store/saryotsujiri-honten/
- Kyoto International Manga Museum: https://kyotomm.jp/en/opentime-cost/
- Higashi Hongan-ji / Shosei-en: https://www.higashihonganji.or.jp/
- Kennin-ji: https://www.kenninji.jp/access/
- Kuromon Market / Namba Yasaka / Namba Parks / Hozenji / Osaka Museum of Housing and Living: https://osaka-info.jp/en/
- Tsurutontan Soemoncho: https://www.tsurutontan.co.jp/shop/soemoncho/
- Mizuno: https://www.mizuno-osaka.com/sp/global_5.html
- Dotonbori Imai: https://www.d-imai.com/shops/honten/

### Restaurant source links

- Kura: https://shop.kurasushi.co.jp/detail/657
- Sushiro: https://www.akindo-sushiro.co.jp/shop/detail.php?id=2750
- Ramen YUCHO reference: https://tabelog.com/kyoto/A2601/A260201/26042575/
- Genrokuzushi: https://tabelog.com/osaka/A2701/A270202/27017693/
- Rikuro Namba: https://www.rikuro.co.jp/shoplist/134.html
- CHIBO: https://www.chibo.com/shop/detail.php?id=4
- Yamamoto no Hamburg: https://tabelog.com/osaka/A2701/A270201/27154000/
- Daiki Suisan official: https://www.daiki-suisan.co.jp/shop/kaitenzushi/doutonbori/
- Osaka Ohsho official: https://www.osaka-ohsho.com/store/detail.php?area=osaka&id=nipponbashi
- Gyoza no Ohsho Nihombashi-Denden-Town official: https://map.ohsho.co.jp/b/ohsho/info/4874/
- KIX Nishiya official: https://www.kansai-airport.or.jp/en/dine/d091

---

# 4. HARUKA — KIX → Kyoto price comparison

Trip use case: **2026-09-13, Kansai Airport → Kyoto, adult one-way**.

| Seller | Price usable for this exact comparison | Coupon caveat | Decision |
|---|---:|---|---|
| **JR-WEST official** | **¥2,200/adult** | None needed | Exact official baseline / price ceiling |
| **Klook** | **Exact 9/13 KIX→Kyoto final price is not exposed in the static public page** | Package/date selection and checkout required | Compare final all-in price; buy only if below ¥2,200 |
| **KKday** | **Exact 9/13 KIX→Kyoto final price is not exposed in the static public page** | App first-purchase promotion can advertise up to 10%; eligibility/final discount must be confirmed at checkout | Compare final all-in price; buy only if below ¥2,200 |
| **Trip.com** | Public page has a generic “from” price, **not an exact KIX→Kyoto route/date price** | Headline not comparable | Do not call it cheaper until package selection confirms exact route |

### HARUKA decision

- **PURCHASED 2026-09-11 for both Oosu + Domenic.**
- Price-shopping is finished; only the QR / exchange / boarding instructions need to be saved offline on 9/12 night.
- HARUKA remains the preferred arrival transport because luggage + likely rain + a direct ride to Kyoto are worth more here than saving a small amount through multiple local-train transfers.

Official JR-WEST: https://www.westjr.co.jp/global/en/ticket/westqr/haruka/

---

# 5. Japan eSIM comparison — two separate travelers

Target: **5-day Japan trip**, each traveler uses their own eSIM.  
Normal recommendation: **5GB each** for maps, translation, web, mytrip.oosu.dev and light social use.  
Ultra-budget: **3GB each** only if video and large uploads stay on hotel Wi-Fi.

## 3GB options

| Provider | Data / validity | Current published price | Network | Tethering | Activation | Verdict |
|---|---|---:|---|---|---|---|
| **TravelSim Asia** | 3GB / 15d | **US$6.99** | NTT Docomo / Rakuten / KDDI au / SoftBank listed | Yes | Install before travel; activation window published by provider | Cheapest published candidate; smaller brand |
| **Nomad** | 3GB / 30d | **US$7.00** | KDDI au / SoftBank | Yes | QR install; plan starts when connecting in destination; activate within provider window | Best clean budget pick |
| **Ubigi** | 3GB / 15d | **US$7.50 / about ¥1,300** | KDDI + NTT Docomo | Data sharing supported | Smartstart on destination connection; pre-install | Strong reliability/value |
| **Saily** | 3GB / 30d | **US$7.99** | KDDI / SoftBank listed | Yes | Install in app; activate within provider window | Fine UX, no price win |
| **Airalo** | 3GB / 30d | **about US$8** | Japan package lists SoftBank + another available network | Supported when device/network allows | Install before departure, connects/activates on arrival | Familiar brand, slightly higher price |

## Selected 5GB option + backups

| Provider | Data / validity | Current published price | Network | Tethering | Activation | Verdict |
|---|---|---:|---|---|---|---|
| **Stellar eSIM** | **5GB / 20d** | **US$1.75/person in the captured offer** | 5G shown on captured offer | Check plan details at checkout | Pre-install before departure | **Selected primary: buy one each for Oosu + Domenic** |
| **Eskimo affiliate freebie** | **1GB / no expiry** | **Free** | Global coverage includes Japan | Provider supports hotspot on fixed-data plans | New user through an eligible affiliate link | **Backup #1: only claim when the official signup screen explicitly shows free 1GB** |
| **Nomad Trial** | **1GB / 3d** | **Free** | Japan included | Yes | Nomad app; new user; no credit card | **Backup #2: 3-day emergency data** |
| **Nomad paid** | 5GB / 30d | US$10 | KDDI au + SoftBank | Yes | QR install | Paid fallback only |
| **Ubigi** | 5GB / 15d | about ¥1,700 | KDDI + NTT Docomo | Yes | Smartstart | Paid fallback only |
| **Saily** | 5GB / 30d | US$10.99 | KDDI / SoftBank listed | Yes | App install | Paid fallback only |

### eSIM recommendation

1. **Stellar 5GB / 20d / US$1.75 each** — selected primary, one eSIM per traveler.
2. **Eskimo affiliate 1GB / no expiry / free** — backup #1; affiliate-link signup screen must show the 1GB offer before claiming.
3. **Nomad Trial 1GB / 3d / free** — backup #2; Japan is included and no credit card is required.

Keep the two free backups unactivated unless the Stellar line fails or runs out.

### eSIM checklist before payment

- Confirm both phones support eSIM and are carrier-unlocked.
- Check EID / eSIM menu on each phone.
- Install profiles in Korea on Wi-Fi.
- Keep the Japan data line off until the provider’s activation instructions say otherwise.
- Save QR codes and APN/activation instructions offline.
- Verify hotspot/tethering if one phone may temporarily share data.

### eSIM / connectivity options rejected

- **Premium unlimited eSIM:** overkill for 5 days; fixed 5GB plans around US$10 are enough for this usage.
- **Pocket Wi-Fi:** extra device to carry/charge/return; poor fit for two people who may separate briefly and each need maps.
- **Korean carrier roaming:** convenient but normally loses the price comparison against these fixed-data travel eSIMs; only reconsider if an existing plan includes cheap/free roaming.
- **Ultra-cheap marketplace “from” listings:** usually compare tiny 1-day/500MB packages rather than a true 5-day 3–5GB plan; reject headline-price comparisons.

Provider pages:

- Nomad: https://www.nomadesim.com/japan-eSIM/
- Nomad Trial: https://www.nomadesim.com/documents/landing-trial-plan
- Eskimo affiliate program: https://www.eskimo.travel/en/affiliate
- Stellar comparison page: https://esimdb.com/japan/stellar
- Ubigi: https://cellulardata.ubigi.com/rates-and-coverage/japan-data-plans/
- Airalo: https://www.airalo.com/japan-esim
- Saily: https://saily.com/esim-japan/
- TravelSim Asia: https://travelsimasia.com/products/japan

---

# 6. ICOCA and transport-pass decision

## Default

Use **pay-as-you-go ICOCA / compatible IC transit payment** for local transport.

- Physical ICOCA standard purchase is typically **¥2,000**, including **¥1,500 usable balance + ¥500 deposit**.
- If a compatible iPhone/Apple Watch setup is already practical, mobile IC can reduce card handling.
- Keep some yen cash because physical IC purchase/top-up situations can still require cash.
- Kyoto City Bus flat-fare-area rides are currently **¥230/adult**. The **Subway & Bus 1-Day Pass is ¥1,100/adult**; the old bus-only 1-day pass has been discontinued.
- For this itinerary the ¥1,100 day pass is not expected to pay off: 9/13 needs only the Kyoto Station→Kawaramachi Gojo bus, 9/14 uses only a small number of city-bus rides, and 9/15 relies mainly on **Keihan**, which is outside the Kyoto City Subway & Bus pass.
- Ordinary automatic IC transfer discounts ended in 2023. Kyoto now has an ICOCA/PiTaPa points program, but it is not worth optimizing around for this short tourist itinerary.

## Contactless bank-card tap: useful, but not a full ICOCA replacement

- **Osaka Metro:** supported. Contactless Visa / Mastercard / JCB / American Express / Diners / Discover / UnionPay cards and compatible phone/watch wallets can be tapped at supported readers without prior registration.
- **Nankai Railway:** supported at designated stations using dedicated touch-payment gates. Confirm the entry and exit stations are both supported before relying on it; this is useful for the Tengachaya↔KIX leg when the dedicated readers are available.
- **Kyoto City Bus / Kyoto Subway:** **not supported yet as of 2026-09**. Kyoto is working toward credit-card contactless payment during FY2027; current official guidance still says bank/credit cards cannot be used directly for bus fare, and subway credit-card tap is unavailable.
- **Keihan Railway:** **not supported** for ordinary fare payment by contactless credit card.
- **JR-WEST / HARUKA:** use the HARUKA ticket/QR and ICOCA-compatible transport IC where applicable; do not assume a normal bank-card tap works at JR-West gates.

**Decision for this trip:** a contactless bank card can reduce ICOCA usage in Osaka, but because Kyoto City Bus/Subway and Keihan are part of the actual itinerary, **ICOCA (or another nationwide-compatible transit IC) remains the simplest all-route local-payment fallback**. Do not buy a Kyoto/Kansai unlimited pass solely to avoid carrying ICOCA.

## Pass options shown side-by-side in MyTrip

The site now separates **Kyoto transport / Osaka transport / eSIM** into independent comparison tables with live purchase/info links instead of grouping them under one “connectivity” card.

- **Kyoto Subway & Bus 1-Day Pass:** official adult price **¥1,100**.
- **Klook Kyoto Subway & Bus 1-Day:** currently shown at **₩11,100/adult**; the voucher must be exchanged at the **Limon Welcome Desk in KIX Terminal 1**, so it is not simply a mobile ticket to buy after reaching Kyoto. Compare its final KRW price with official ¥1,100.
- **Keihan Kyoto Sightseeing Pass:** **¥1,100 1-day / ¥1,300 24h**.
- **Keihan Kyoto-Osaka Sightseeing Pass:** **¥1,650 1-day / ¥1,850 24h**.
- **Osaka Enjoy Eco Card:** **¥820 weekday / ¥620 weekends & holidays**.
- **Klook Osaka Metro + City Bus 1-Day QR:** currently **₩8,900/adult**.
- **Osaka Amazing Pass:** **¥3,500/1-day**; not a value fit for the current low-attraction-count itinerary.

For the current itinerary the site recommends **ICOCA/pay-as-you-go in Kyoto** and **card tap or ICOCA/pay-as-you-go in Osaka**, while still making each pass purchasable from the comparison table if the plan changes.

## Do not pre-buy a broad Kansai transport pass

This itinerary does not contain enough expensive long-distance rides to justify a wide unlimited pass. Main movements are:

- one HARUKA airport ride,
- inexpensive Kyoto city rides,
- one Kyoto→Osaka ordinary rail transfer,
- Osaka Metro,
- one Nankai Airport Express ride.

JR-WEST IC guide: https://www.westjr.co.jp/travel-information/en/train-usage-guide/howto/guide/

---

# 7. Transport master plan

## KIX → Kyoto — 9/13

1. KIX immigration/baggage.
2. JR **HARUKA → Kyoto Station**.
3. Default with luggage: Kyoto City Bus **4 / 5 / 80 / 205 → Kawaramachi Gojo**.
4. Walk about **1 minute** to AMANEK.

Hotel-official fallback if buses are badly delayed/crowded: Kyoto Subway Karasuma Line **Kyoto → Gojo** (1 stop), then about **10 minutes on foot**.

No taxi. HARUKA keeps transfers low on the rainy luggage day.

## Kyoto local — 9/13–9/15

- Kiyomizu: city bus + walk.
- Gion/Kawaramachi/Teramachi: walking + short bus as needed.
- Conditional Fushimi Inari: **Keihan Kiyomizu-Gojo ↔ Fushimi-Inari**.
- Use ICOCA/pay-as-you-go rather than a tourist pass.

## Kyoto → Osaka — 9/15

Preferred budget flow from the hotel area:

1. Kiyomizu-Gojo → Kitahama direction by **Keihan**; use the appropriate service/transfer for the departure time.
2. Kitahama → **Ebisucho** by Osaka Metro Sakaisuji Line.
3. Walk to Nipponbashi Crystal Hotel.

**Rejected:** Shinkansen. It costs more and creates unnecessary station access for this hotel-to-hotel move.

## Osaka local — 9/15–9/16

- Nipponbashi/Ebisucho ↔ Namba/Dotonbori: mostly walk.
- Osaka Castle: Osaka Metro, then walk inside the park.
- Shinsaibashi → Dotonbori: covered arcade on foot.

## Osaka → KIX — 9/17

1. **Ebisucho → Tengachaya** on Osaka Metro Sakaisuji Line.
2. **Tengachaya → KIX** on Nankai Airport Express.
3. Target KIX T1 around 08:45–09:00.

**Rapi:t is not the default.** Pay the premium only if the regular Airport Express is disrupted or the time buffer becomes unsafe.

---

# 8. AMANEK public bath

Do not search for an external tattoo sauna or alternative hotel.

- Existing hotel information used for planning: **evening 16:00–24:00; morning 06:00–09:00**.
- Traveler reviews checked during planning include tattoo-friendly experiences, but guest reviews are not a substitute for a permanent hotel-policy guarantee.
- The trip deliberately uses evening slots so the schedule does not depend on morning access.

### Fixed itinerary slots

- **9/13 20:30–21:30**
- **9/14 16:30–17:30**

---

# 9. Attractions and ticket decisions

| Place | Reservation? | Decision |
|---|---|---|
| Kiyomizu-dera | No advance reservation needed | Buy on site; core Kyoto sight |
| Fushimi Inari | No; free | Only lower area if rain is light |
| Nishiki / Teramachi / Shinkyogoku | No | Covered/food-oriented arrival-day and rain fallback |
| Gion | No | Pair with Kiyomizu; cut walking in heavy rain |
| Dotonbori | No | Repeat food/snack/shopping base |
| Osaka Castle | Advance booking not essential for this plan | Buy on the day to preserve weather flexibility |
| Arashiyama | — | Excluded from core plan |
| Kinkaku-ji | — | Excluded from core plan |

---

# 10. Visit Japan Web

Complete before departure for **each traveler**.

1. Register/login at the official Visit Japan Web service.
2. Register the trip/arrival information.
3. Complete available immigration/customs arrival procedures.
4. Save the resulting QR screen(s) offline on each phone.

Official: https://www.vjw.digital.go.jp/

Do not rely on airport cellular data for the only copy of the QR.

---

# 11. Travel insurance

Travel insurance is not treated as an optional afterthought even on a budget trip.

- First check whether a credit card already provides overseas medical/emergency coverage.
- Avoid paying twice for the same coverage.
- If coverage is weak/absent, buy a normal short Japan policy before departure.
- Save insurer emergency contact and policy access offline.
- Prefer a policy with meaningful overseas medical treatment coverage and a clear emergency-assistance process.

JNTO: https://www.japan.travel/en/plan/travel-insurance-in-japan/

---

# 12. What to prepare in Korea

## Must / strongly recommended

- [ ] Japan **Type-A plug adapters (“돼지코”) — 2+**
- [ ] Confirm every charger says **INPUT 100–240V**; Japan mains power is 100V
- [ ] Compact umbrella
- [ ] Thin waterproof shell / quick-drying outer layer
- [ ] Broken-in walking shoes
- [ ] Blister plasters / small foot-care kit
- [ ] Power bank + charging cables; power bank stays in carry-on
- [ ] Small waterproof zip pouch for passport/electronics
- [ ] **Sony mirrorless camera** + battery / memory card / charger check
- [ ] **Neck pillow**
- [ ] Normal personal medicine in appropriate packaging
- [ ] Two eSIMs + offline QR/setup screenshots
- [ ] Small amount of yen cash + card(s)

## Do not overbuy

- Large shampoo/body-wash bottles: hotels supply basics.
- Large food/drink stock: convenience stores are everywhere.
- Heavy raincoat: September is warm/humid; use a light layer.
- Huge cash amount: cards + IC transit are useful; keep cash as backup, not the only method.

Japan electricity reference: https://www.japan.travel/en/plan/plug-and-electricity/

---

# 13. Departure checklist

## 9/10–9/11

- [x] Compare **final checkout** KIX→Kyoto adult 9/13 price and choose seller
- [x] Buy HARUKA for **Oosu + Domenic**
- [ ] Buy **Stellar 5GB / 20d eSIM × 2 (US$1.75 each offer)**
- [ ] Claim optional backup: **Eskimo affiliate free 1GB / no expiry**
- [ ] Claim optional backup: **Nomad Trial free 1GB / 3d**
- [ ] Install both eSIM profiles on Wi-Fi
- [ ] Reserve Kura Sushi Kyoto Teramachi — 9/13 ~16:30
- [ ] Reserve Sushiro Kyoto Gion — 9/14 ~11:15
- [ ] Reserve CHIBO Dotonbori — 9/15 ~18:30
- [ ] Complete Visit Japan Web for both travelers
- [ ] Check/buy travel insurance
- [ ] Buy Type-A plug adapters if missing
- [ ] Prepare umbrella + waterproof outer layer
- [ ] Buy **tattoo-cover leg gaiter / sleeve (발토시)** for AMANEK bath use if needed

## 9/12 night

- [ ] Screenshot flight confirmations
- [ ] Screenshot hotel confirmations and addresses
- [ ] Screenshot HARUKA QR/exchange instructions
- [ ] Screenshot eSIM setup instructions
- [ ] Screenshot Visit Japan Web QR(s)
- [ ] Screenshot insurance emergency contact/policy access
- [ ] Charge power banks
- [ ] Confirm `https://mytrip.oosu.dev` opens on both phones
- [ ] Download useful offline maps if desired
- [ ] Check final hourly Kyoto rain forecast
- [ ] Recheck restaurant temporary closure notices, especially official pages

---

# 14. Rejected / backup options and reasons

## Sights rejected from the core plan

### Arashiyama

Long cross-city travel + more walking + wet paths. Poor trade for a short, rainy, food-first trip.

### Kinkaku-ji

Inefficient from the hotel/Kiyomizu-Gion route; bus time adds up without improving the core trip enough.

### Full Fushimi Inari mountain climb

Excess walking and slippery-rain risk. Only the lower shrine/Senbon Torii entrance remains conditional.

### Extra Kyoto temple checklist

Direct conflict with the 10k-step ceiling and food/rest priority.

## Transport rejected

### Taxi

Hard reject under the budget rule.

### Shinkansen Kyoto → Osaka

Unnecessary cost for this short hotel-to-hotel movement.

### Nankai Rapi:t as the default

Regular Airport Express is adequate and cheaper. Rapi:t remains an emergency timing option only.

### Kansai-wide tourist rail pass

Not enough expensive rides to recover the pass price.

## Food rejected

- High-end omakase: outside the sushi budget/food-hopping style.
- Uni/shellfish-heavy seafood shops: poor fit for Domenic.
- Very spicy ramen/specialties: poor fit for Oosu.
- Any restaurant requiring a long detour solely to add a “famous” place: conflicts with the low-walking plan.

## Connectivity rejected

- Unlimited premium eSIM: unnecessary cost for 5 days.
- Pocket Wi-Fi: extra battery/device/return burden.
- Korean carrier roaming as the default: generally weaker price/value than the fixed 5GB eSIM candidates unless already bundled in a plan.
- Tiny marketplace teaser plans: not comparable to the required 3–5GB trip package.

---

# 15. Mobile site data contract — mytrip.oosu.dev

The production site should expose this plan as structured SQLite data rather than requiring this Markdown file on the phone.

## Before departure

Checkable status for:

- HARUKA purchase
- eSIM purchase
- eSIM installation
- Visit Japan Web
- travel insurance
- Type-A plug adapters
- umbrella / waterproof layer
- restaurant reservations
- offline screenshots / phone readiness

## Daily itinerary

For every date show:

- time
- place / food
- transport mode
- estimated walking target
- rain fallback

## Restaurant info

For every restaurant show:

- current hours
- price range
- RESERVE NOW / RESERVATION OPTIONAL / WALK-IN ONLY
- current TODO / BOOKED / WALK-IN status
- reservation channel
- dietary warning

## Transport

Keep quick-reference cards for:

- KIX → Kyoto
- Kyoto local
- Kyoto → Osaka
- Osaka local
- Osaka → KIX

## Packing

Keep existing packing checkboxes and add trip-specific items such as Type-A adapters, waterproof layer, blister care and waterproof pouch without deleting existing user state.

---

# 16. Sources and verification notes

This document was rebuilt on **2026-09-10** from confirmed trip data, prior planning material and a fresh web verification pass. Dynamic checkout prices and temporary restaurant closures can change after this timestamp.

Priority rule used for restaurant facts:

1. official store / official booking system,
2. airport/operator official page,
3. Tabelog or other listing when official reservation details are unavailable.

Priority rule used for prices:

- Do not treat a “from” headline as the price for **KIX → Kyoto / adult / 9/13**.
- If a reseller hides the package-specific price until selection/checkout, record that limitation instead of inventing a price.

This is deliberate: **an unknown checkout price is better than a false cheapest-price claim.**

...
{"message":"A short 1–3 sentence takeaway, no giant paragraph and no numbered list embedded in this string.","sections":[{"title":"Short section heading","items":["One concise actionable item","Another concise item"]}],"ideas":[{"name":"...","category":"...","reason":"...","bestTime":"...","area":"..."}],"actions":[{"type":"event_status","event_id":"an exact itinerary id","status":"DONE|SKIPPED|CANCELLED|PLANNED","label":"short Korean confirmation label"},{"type":"select_meal","slot_id":"an exact meal id","restaurant_id":"an exact option id","label":"short Korean confirmation label"},{"type":"select_decision","slot_id":"an exact day_decision id","option_id":"an exact option id","label":"short Korean confirmation label"}]}.

Use 1–3 sections when the answer benefits from structure (for example 권장 순서, 주의할 점, 이동/식사). Put ordered recommendations into sections.items rather than packing '1) 2) 3)' into message. Keep each item concise enough to scan on a phone. sections may be an empty array for a simple answer. ideas may be an empty array when the question is informational; otherwise return at most 6 actionable candidate ideas. actions may be an empty array. Never invent an id: only emit an action when every referenced id exists exactly in the supplied context and the change directly answers the user's request or a clearly explained recommendation. The client will still require the user to press an apply button before any action runs.`,

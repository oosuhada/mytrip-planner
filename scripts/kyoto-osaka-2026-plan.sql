...
VALUES
('plan-event-0913-home-arex', (SELECT id FROM trips WHERE title='Kyoto · Osaka 2026' LIMIT 1), '집 → 홍대입구역 AREX', 'transfer', '2026-09-13', '05:08', '05:20', '서울 마포구 와우산로38길 10 → Hongik University Station', '서울 마포구 와우산로38길 10', NULL, NULL, '동교동 188-6에서 홍대입구역까지 도보 약 7분권. 캐리어와 개찰구 이동을 감안해 12분 확보.', 'travel-plan-2026', 1, '{"transport":"walk","daily_walking":"6k–8k steps","walking":"약 10–12분","rain":"비 오면 우산·캐리어 방수커버 사용"}'),
('plan-event-0913-arex', (SELECT id FROM trips WHERE title='Kyoto · Osaka 2026' LIMIT 1), 'AREX 첫차 · 홍대입구 → ICN Terminal 1', 'train', '2026-09-13', '05:28', '06:22', 'Hongik University Station → Incheon International Airport Terminal 1', NULL, NULL, NULL, '공항철도 일반열차 첫차. 현재 시간표 기준 05:28 출발, T1 약 06:22 도착. 05:47 열차는 fallback, 05:57 이후는 08:00 출발편에 여유가 적어 비추천.', 'travel-plan-2026', 2, '{"transport":"AREX all-stop train","walking":"역 환승만","rain":"철도 이동"}'),
('plan-event-0913-icn-process', (SELECT id FROM trips WHERE title='Kyoto · Osaka 2026' LIMIT 1), 'ICN T1 · 출국수속 · 보안검색 · 게이트 이동', 'activity', '2026-09-13', '06:22', '07:35', 'Incheon International Airport Terminal 1', NULL, NULL, NULL, '온라인 체크인 완료 + 별도 환전/카운터 업무 없음 전제. 공항 도착 후 바로 출국장 보안검색 → 출국심사 → 게이트. 주말 보안검색 혼잡은 변수.', 'travel-plan-2026', 3, '{"transport":"airport walk","walking":"공항 내부","rain":"완전 실내"}'),
...
('plan-event-0913-hotel-transfer', (SELECT id FROM trips WHERE title='Kyoto · Osaka 2026' LIMIT 1), 'Kyoto Station → Gojo Station', 'train', '2026-09-13', '12:35', '12:50', 'Kyoto Station → Gojo Station', NULL, NULL, NULL, 'Kyoto Subway Karasuma Line 1정거장. 플랫폼 이동/대기 포함 15분 확보.', 'travel-plan-2026', 20, '{"transport":"Kyoto Subway Karasuma Line","walking":"역 내부","rain":"실내 중심"}'),
('plan-event-0913-gojo-amanek-walk', (SELECT id FROM trips WHERE title='Kyoto · Osaka 2026' LIMIT 1), 'Gojo Station → HOTEL AMANEK', 'transfer', '2026-09-13', '12:50', '13:05', 'Gojo Station → HOTEL AMANEK Kyoto Kawaramachi Gojo', '616 Azuchicho, Shimogyo Ward, Kyoto', 34.9951, 135.7654, 'Gojo역에서 호텔까지 도보. 택시 사용하지 않음.', 'travel-plan-2026', 21, '{"transport":"walk","walking":"약 10–15분","rain":"우산 사용; 큰 비면 지하철 출구에서 방수 준비"}'),
